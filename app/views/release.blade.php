...
<div class="panel" ng-controller="ReleaseController" ng-show="loaded" ng-cloak>
  <div class="row">
    ...
    <div class="large-10 columns"><h1>@{{ release_data.album }}</h1><h3>@{{ artist }}, Released: @{{ release_data.date }}</h3></div>
  </div>
  ...
  <div class="tracklist-row">
    <div class="panel">
      <h3>Tracklist</h3>
      <ul class="no-bullet">
        <li ng-repeat="track in release_data.tracks"><b>@{{ ($index + 1) }}.</b> @{{ track.title }}</li>
      </ul>
    </div>
  </div>
</div>
